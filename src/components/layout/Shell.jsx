      ...
      <div className="lg:hidden flex flex-col min-h-[100dvh]">
        ...
        {mob && (
          <div className="fixed inset-0 z-[60]">
            ...
            <aside className="absolute left-0 top-0 bottom-0 w-64 bg-white flex flex-col animate-slide-in shadow-2xl border-r border-slate-200">
              <div className="px-4 pt-4 pb-3 flex items-center justify-between border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-[13px] font-semibold text-slate-900">TeleInspect</div>
                    <div className="text-[10px] text-slate-400">Auditoría</div>
                  </div>
                </div>
                <button onClick={() => setMob(false)} className="text-slate-400 hover:text-slate-700 p-1 rounded-md hover:bg-slate-100">
                  <X size={17} />
                </button>
              </div>
              ...
              <div className="px-3 pb-5 border-t border-slate-100 pt-3 mt-2 space-y-1">
                <div className="flex items-center gap-2.5 px-3 py-2 mb-1">
                  <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0">
                    <span className="text-indigo-600 text-[9px] font-bold uppercase">{(user?.name || 'U')[0]}</span>
                  </div>
                  <div className="min-w-0">
                    <div className="text-[11px] font-semibold text-slate-700 truncate">{user?.name || 'Usuario'}</div>
                    <div className="text-[10px] text-slate-400">{user?.role || ''}</div>
                  </div>
                </div>
                <button onClick={() => { logout(); navigate('/login'); setMob(false) }}
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12px] text-slate-500 hover:text-red-600 hover:bg-red-50 transition-colors">
                  <LogOut size={14} />Salir
                </button>
                <div className="text-[9px] text-slate-400 px-3 pt-1">v{APP_VERSION}</div>
              </div>
            </aside>
          </div>
        )}
