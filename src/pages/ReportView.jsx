      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="mt-6 border border-primary/10 rounded-3xl p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-xs text-primary/60 font-bold">Informe (Demo UI)</div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.entries(order.fields).map(([k, v]) => (
                <div key={k} className="rounded-3xl border border-primary/10 p-3">
                  ...
                  <div className="text-sm font-extrabold text-primary mt-1">{String(v)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 text-[11px] text-primary/50">
            Nota: En v1.1.0 este “informe” es una vista imprimible. Cuando conectemos backend, aquí incluiremos la plantilla real del reporte y el botón “Descargar” traerá el PDF definitivo.
          </div>
        </div>
      </div>
