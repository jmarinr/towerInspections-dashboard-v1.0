...
function CompanyModal({ company, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="rounded-2xl w-full max-w-md" style={{ background:'var(--bg-card)', border:'1px solid var(--border)' }} onClick={e=>e.stopPropagation()}>
        <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom:'1px solid var(--border)' }}>
          <h2 className="text-[15px] font-semibold th-text-p">{company ? 'Editar empresa' : 'Nueva empresa'}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg th-text-m" style={{ background:'var(--bg-base)' }}><X size={15}/></button>
        </div>
        <div className="p-5 space-y-4">
          {error && <div className="text-[12px] text-bad px-3 py-2 rounded-lg" style={{ background:'var(--bg-base)' }}>{error}</div>}
          {[
        ...
        <div className="px-5 pb-5 flex gap-2 justify-end">
          <button onClick={onClose} className="h-9 px-4 rounded-lg text-[13px] th-text-s transition-colors" style={{ background:'var(--bg-base)', border:'1px solid var(--border)' }}>Cancelar</button>
          ...
          <table className="w-full text-[13px]" style={{ borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ borderBottom:'1px solid var(--border-light)' }}>
                {['Empresa','Código','País','Estado',''].map(h=>(
            ...
            <tbody>
              ...
              {!companies.length && (
                <tr><td colSpan="5" className="px-4 py-12 text-center text-[13px] th-text-m">Sin empresas registradas</td></tr>
              )}
            </tbody>
          </table>
        </div>
