      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <BarChart data={durationBuckets}>
              ...
              <Bar dataKey="count" name="Órdenes" radius={[4,4,0,0]}>
                {durationBuckets.map((b, i) => (
                  <Cell key={i} fill={b.label.includes('>') ? '#dc2626' : b.label.includes('3d') ? '#ca8a04' : '#0284C7'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="rounded-2xl p-5 th-shadow flex flex-col gap-3" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          ...
          <button onClick={exportToExcel}
            className="px-4 py-2 rounded-xl text-[13px] font-semibold text-white flex items-center justify-center gap-1.5"
            ...
            {paginated.map((v, i) => (
              <tr key={v.id || i} style={{ borderTop: '1px solid var(--border-light)' }}
                ...
                <td className="px-4 py-3 font-mono font-bold text-[13px]"
                  style={{ color: v.status === 'closed' ? 'var(--text-secondary)' : v.slaStatus === 'alert' ? '#dc2626' : v.slaStatus === 'warn' ? '#ca8a04' : 'var(--text-secondary)' }}>
