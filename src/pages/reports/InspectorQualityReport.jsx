...
function StatCell({ value, color }) {
  return <span className="font-mono font-bold text-[13px]" style={{ color }}>{value}%</span>
}
...
export default function InspectorQualityReport() {
  const {
    filtered, paginated, kpis, orgs,
    filterOrg, setFilter,
    exportToExcel, isLoading, error,
  } = useInspectorQualityReport()
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Cards de inspectores */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.filter(i => i.orders > 0).map(ins => (
          <div key={ins.username || ins.name}
            className="rounded-2xl p-5 th-shadow"
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="ml-auto text-[22px] font-bold flex-shrink-0" style={{ color: scoreColor(ins.qualityScore) }}>
                {ins.qualityScore}
              </div>
            </div>
              ...
              <div key={label} className="mb-3">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <StatCell value={value} color={rateColor(value)} />
                  </div>
                </div>
            ...
            {ins.avgDaysOpenVal !== null && (
              <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--border-light)' }}>
                ...
                <span className="font-mono font-bold text-[13px]" style={{ color: ins.avgDaysOpenVal > 7 ? '#dc2626' : ins.avgDaysOpenVal > 3 ? '#ca8a04' : '#16a34a' }}>
                  {ins.avgDaysOpenVal}d
                </span>
              </div>
            )}
