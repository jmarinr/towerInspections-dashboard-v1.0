...
function SiteRow({ site }) {
  ...
  return (
    <>
      <tr onClick={() => setExpanded(!expanded)} style={{ borderTop:'1px solid var(--border-light)', cursor:'pointer' }}
        onMouseEnter={e => e.currentTarget.style.background='var(--row-hover-bg)'}
        onMouseLeave={e => e.currentTarget.style.background=''}>
        <td className="px-4 py-3">{expanded ? <ChevronDown size={14} className="th-text-m" /> : <ChevronRight size={14} className="th-text-m" />}</td>
            ...
            <div className="space-y-2">
              {site.visits.map((v, idx) => (
                <div key={v.visitId || idx} className="flex items-center gap-4 text-[12px]"
                  ...
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0" style={{ background:'var(--accent)', color:'#fff' }}>{idx+1}</span>
                  <span className="th-text-m w-24 flex-shrink-0">{v.startedAt ? new Date(v.startedAt).toLocaleDateString('es') : '—'}</span>
                  <span className="px-2 py-0.5 rounded text-[11px] font-bold flex-shrink-0" style={{ background:v.status==='closed'?'#dcfce7':'#dbeafe', color:v.status==='closed'?'#15803d':'#1d4ed8' }}>{v.status}</span>
                  ...
                  {idx > 0 && <ImprovBadge improvement={v.rate - site.firstV.rate} />}
                </div>
              ))}
            </div>
          </td>
...
export default function HistorialSitiosReport({ hook }) {
  const { filtered, paginated, kpis, orgs, filterOrg, filterMin, search, setFilter, currentPage, setCurrentPage, pageSize, totalFiltered, isLoading, error } = hook
