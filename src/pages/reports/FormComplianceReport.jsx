...
const FORM_COLORS = {
  ...
  'mantenimiento-ejecutado': '#0891b2',
  'inventario-v2':           '#7c3aed',
  'sistema-ascenso':         '#16a34a',
  'additional-photo-report': '#dc2626',
  'puesta-tierra':           '#d97706',
  'inspeccion':              '#64748b',
}
...
export default function FormComplianceReport() {
  const {
    byForm, filteredTrend, kpis, formCodes,
    filterForm, setFilterForm,
    exportToExcel, isLoading, error,
    FORM_LABELS,
  } = useFormComplianceReport()
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Barras de completitud por formulario */}
      <div className="rounded-2xl p-5 th-shadow" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[14px] font-bold th-text-p">Tasa de Completitud por Formulario</h3>
          <button onClick={exportToExcel}
            className="px-3 py-1.5 rounded-xl text-[12px] font-semibold text-white flex items-center gap-1"
          ...
          {[...byForm].sort((a,b) => b.rate - a.rate).map(f => (
            <div key={f.code}>
              ...
              <div style={{ background: 'var(--bg-base)', borderRadius: 6, height: 10, overflow: 'hidden' }}>
                <div style={{ width: `${f.rate}%`, height: '100%', background: rateColor(f.rate), borderRadius: 6, transition: 'width .5s' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-2xl p-5 th-shadow" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            ...
            <select value={filterForm} onChange={e => setFilterForm(e.target.value)}
              ...
              {formCodes.filter(fc => fc !== 'inspeccion').map(fc => (
                <option key={fc} value={fc}>{FORM_LABELS[fc] || fc}</option>
              ))}
            </select>
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={filteredTrend}>
              ...
              <Tooltip
                ...
                formatter={(v, name) => [`${v ?? '—'}%`, FORM_LABELS[name] || name]}
              />
              <Legend formatter={name => FORM_LABELS[name] || name} wrapperStyle={{ fontSize: 11 }} />
              {activeCodes.map(fc => (
                <Line key={fc} type="monotone" dataKey={fc}
                  stroke={FORM_COLORS[fc] || '#94a3b8'}
